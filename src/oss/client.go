...
//
func New(endpoint, accessKeyID, accessKeySecret string, concurrency int32, options ...ClientOption) (*Client, error) {
...
func (client Client) do(method, bucketName, urlParams, subResource string,
	...
	atomic.AddInt32(&conn_cnt, 1)
	defer atomic.AddInt32(&conn_cnt, -1)

	for atomic.LoadInt32(&conn_cnt) > client.Config.Concurrency {
		fmt.Println("oss concurrency full:", client.Config.Concurrency)

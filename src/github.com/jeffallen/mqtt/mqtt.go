...
func statsMessage(topic string, c, cm, mr, ms, con, mps int64) *proto.Publish {
	return &proto.Publish{
		Header:    header(dupFalse, proto.QosAtMostOnce, retainTrue),
		TopicName: topic,
		Payload:   newStringPayload(fmt.Sprintf("active-client:%d,client-max:%d,msg-receive:%d,msg-send:%d,concurrent:%d,msg-per-sec:%d", c, cm, mr, ms, con, mps)),
	}
}
...
func (s *stats) publish(sub *subscriptions, interval time.Duration) {
	...
	sub.submit(nil, statsMessage("mqttserver/stat", clients, clientsMax, atomic.LoadInt64(&s.recv), atomic.LoadInt64(&s.sent), int64(runtime.NumGoroutine()), msgpersec))
}
...
type subscriptions struct {
	...
	wildcards []wild
	retain    map[string]retain //TODO:save more retain msgs
	stats     *stats
}

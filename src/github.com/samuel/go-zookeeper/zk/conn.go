...
func (c *Conn) SafeCloseWithTimeout(seconds time.Duration) {
	select {
	case <-c.queueRequest(opClose, &closeRequest{}, &closeResponse{}, nil):
		close(c.shouldQuit)
...
func (c *Conn) connect() error {
	...
	for {
		...
		go tools.SendInternalMail(appcfg.GetString("odps_env_name", "")+"_zookeeper_down_"+appcfg.GetAddress(), c.Server()+"<br>"+err.Error(), nil)
	}
}
...
func (c *Conn) Busy() bool {
	return len(c.sendChan) > sendChanSize/2
}
